...
export const StringInput: React.FC<StringInputProps> = (props) => {
  ...
  return (
    <div className={`input-container ${config.fill && 'fill'}`}>
      <h5>{config.label}:</h5>
      <input onChange={(e) => props.updateFunc(config.name, e.target.value)} onFocus={() => setVisited(true)} placeholder={config.placeholder} type="text" id={config.name} name={config.name} />
